...
const Breadcrumbs = ({ breadcrumbs }: { breadcrumbs: Breadcrumb[] }) => (
    breadcrumbs.map((breadcrumb, index) => (<div className="inline-block" key={index}>
        ...
        {breadcrumb.link ? <Link href={breadcrumb.link} className="inline-block align-text-top">{breadcrumb.text}</Link> : <span className="inline-block align-text-top">{breadcrumb.text}</span>}
    </div>))
)
